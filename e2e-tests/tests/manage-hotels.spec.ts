...
test ("should edit hotel", async({page})=>{
    ...
    await expect(page.locator('[name="name"]')).toHaveValue("novanta");
    await page.locator('[name="name"]').fill("Novanta v4");
    await page.getByRole("button", {name: "Save"}).click();
    await expect (page.getByText("Hotel Saved!")).toBeVisible();
    
    await page.reload();

    await expect(page.locator('[name="name"]')).toHaveValue("Novanta v4");

    await page.locator('[name="name"]').fill("novanta");
    await page.getByRole("button", {name: "Save"}).click();
